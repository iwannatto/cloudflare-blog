...
void netns_new() {
    ...
    {
        // ip link set lo up
        char data1[] = "\x00\x00\x00\x00\x01\x00\x00\x00\x01\x00\x00\x00\x01"
                       "\x00\x00\x00";

        char buf[128];
        struct nlmsghdr *nlmsg = (void *)buf;
        *nlmsg = (struct nlmsghdr){
            .nlmsg_len = sizeof(data1) - 1 + sizeof(struct nlmsghdr),
            .nlmsg_type = 0x10,
    ...
    }
    {
        ...
        char data2[] = "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
        ...
        struct nlmsghdr *nlmsg = (void *)buf;
        *nlmsg = (struct nlmsghdr){
            .nlmsg_len = sizeof(data2) - 1 + sizeof(struct nlmsghdr),
            .nlmsg_type = 0x10,
...

    {
        ...
        char data3[] = "\x00\x00\x00\x00\x02\x00\x00\x00\x01\x00\x00\x00\x01"
                       "\x00\x00\x00";

        char buf[128];
        struct nlmsghdr *nlmsg = (void *)buf;
        *nlmsg = (struct nlmsghdr){
            .nlmsg_len = sizeof(data3) - 1 + sizeof(struct nlmsghdr),
            .nlmsg_type = 0x10,
            .nlmsg_flags = NLM_F_REQUEST,
            .nlmsg_seq = 1,
        };
