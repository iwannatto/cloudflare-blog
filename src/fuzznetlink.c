...
int main(int argc, char **argv) {
    ...
    while (1) {
        int option_index = 0;
        int arg =
            getopt_long(argc, argv, optstring, long_options, &option_index);
        if (arg == -1) {
            break;
        }
    ...
    if (state->user_dmesg) {
        user_dmesg_fs =
            open(state->user_dmesg, O_APPEND | O_WRONLY | O_CREAT, 0644);
        if (user_dmesg_fs < 0) {
    ...
    for (run_no = 0; 1; run_no += 1) {
        ...
        int netlink_fd = socket(AF_NETLINK, SOCK_RAW | SOCK_NONBLOCK, buf[0]);
        if (netlink_fd < 0) {
            goto error;
        }
    ...
    error:
        ...
        while (1) {
            ...
            if (strstr(buf, "Call Trace") != NULL ||
                strstr(buf, "RIP:") != NULL || strstr(buf, "Code:") != NULL) {
                crashed += 1;
            }
        }
        if (crashed) {
